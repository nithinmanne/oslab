...
private:
    ...
    public:
        ...
        inline block* operator[](int i) {
            if(i<0) return list[numblocks-i];
            return list[i];
        }
    };
    int appendl(indexnode& inode, const char *data, size_t size);
    int deletel(indexnode& inode, int size);
    class fdt {
    public:
        int id;
        int type;
        int offset;
        indexnode &inode;
        fdt(int id, int type, int offset, indexnode &inode);
        fdt(const fdt& other);
        fdt& operator=(const fdt& other);
    };
    int maxfdt;
    std::vector<fdt> fdtlist;
...
public:
    ...
    int write_myfs(int fd, int nbytes, const char *buff);
    int eof_myfs(int fd);
